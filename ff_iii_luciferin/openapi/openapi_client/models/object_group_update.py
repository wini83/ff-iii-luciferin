...
class ObjectGroupUpdate(BaseModel):
    ...
    order: Optional[StrictInt] = Field(
        default=None, description="Order of the object group"
    )
    __properties = ["title", "order"]
    ...
    @classmethod
    def from_json(cls, json_str: str) -> ObjectGroupUpdate:
    ...
    @classmethod
    def from_dict(cls, obj: dict) -> ObjectGroupUpdate:
        """Create an instance of ObjectGroupUpdate from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return ObjectGroupUpdate.parse_obj(obj)

        _obj = ObjectGroupUpdate.parse_obj(
            {"title": obj.get("title"), "order": obj.get("order")}
        )
        return _obj
